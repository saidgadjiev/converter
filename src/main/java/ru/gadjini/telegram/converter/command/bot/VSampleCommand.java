...
public class VSampleCommand implements BotCommand, NavigableBotCommand {
    ...
    public String getCommandIdentifier() {
        return ConverterCommandNames.VSAMPLE;
    }
        ...
        if (existsState == null) {
            ...
            messageService.sendMessage(
                    SendMessage.builder().chatId(String.valueOf(message.getChatId()))
                            .text(localisationService.getMessage(ConverterMessagesProperties.MESSAGE_VSAMPLE_AWAITING_START_POINT, locale))
                            .parseMode(ParseMode.HTML)
                            .replyMarkup(replyKeyboardService.sampleVideoKeyboard(message.getChatId(), locale))
                            .build()
            );
            ...
            if (localisationService.getMessage(ConverterMessagesProperties.START_POINT_COMMAND_NAME, locale).equals(text)) {
                ...
                convertionService.createConversion(message.getFrom(), existsState, Format.SAMPLE, locale);
                commandStateService.deleteState(message.getChatId(), getCommandIdentifier());
            } else if (existsState.getSettings().getCutStartPoint() == null) {
    ...
    private void checkMedia(MessageMedia media, Locale locale) {
        if (media == null || media.getFormat().getCategory() != FormatCategory.VIDEO) {
            throw new UserException(localisationService.getMessage(ConverterMessagesProperties.MESSAGE_VSAMPLE_WELCOME, locale));
        }
    }
    ...
    private Period parsePeriod(String candidate, Locale locale) {
        try {
            ...
            throw new UserException(localisationService.getMessage(ConverterMessagesProperties.MESSAGE_VSAMPLE_AWAITING_START_POINT, locale));
        }
    }
}
