...
public class StartCommand implements NavigableBotCommand, BotCommand, PaidChannelSubscriptionOptional {
    ...
    private void checkSrcFormat(long userId, Format format, String mimeType, String fileName, Locale locale) {
        ...
        if (format.getCategory() == FormatCategory.ARCHIVE) {
            ...
            throw new UserException(localisationService.getMessage(ConverterMessagesProperties.MESSAGE_UNSUPPORTED_FILE, new Object[]{mimeType}, locale));
        }
    ...
    private void checkTargetFormat(long userId, List<MessageMedia> files, Format format, Format target,
        ...
        if (Objects.equals(format, target) && !Format.MP4.equals(target)) {
