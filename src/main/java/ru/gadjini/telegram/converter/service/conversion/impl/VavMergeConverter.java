...
public class VavMergeConverter extends BaseAny2AnyConverter {
    ...
    protected ConversionResult doConvert(ConversionQueueItem conversionQueueItem) {
        ...
        try {
            List<FFprobeDevice.FFProbeStream> streams = fFprobeDevice.getStreams(video.getAbsolutePath(), FormatCategory.VIDEO);
            ...
            if (!audios.isEmpty()) {
                ...
                for (int index = 0; index < audios.size(); index++) {
                    ...
                    List<FFprobeDevice.FFProbeStream> allStreams = fFprobeDevice.getStreams(audio.getAbsolutePath(), FormatCategory.AUDIO);
            ...
            if (!subtitles.isEmpty()) {
                ...
                for (SmartTempFile subtitle : subtitles) {
                    List<FFprobeDevice.FFProbeStream> allStreams = fFprobeDevice.getStreams(subtitle.getAbsolutePath(), FormatCategory.SUBTITLES);
                    for (FFprobeDevice.FFProbeStream stream : allStreams) {
