...
public class Image2SvgConverter extends BaseAny2AnyConverter {
    ...
    public ConvertResult doConvert(ConversionQueueItem fileQueueItem) {
        ...
        try {
            SmartTempFile result = getFileService().createTempFile(fileQueueItem.getUserId(), fileQueueItem.getFirstFileId(), TAG, Format.SVG.getExt());
            try {
                if (fileQueueItem.getFirstFileFormat() != Format.PNG) {
                    SmartTempFile tempFile = getFileService().createTempFile(fileQueueItem.getUserId(), fileQueueItem.getFirstFileId(), TAG, Format.PNG.getExt());
                    try {
